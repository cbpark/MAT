...
std::vector<FourMomentum> onShellSolution(const FourMomentum& vis, double pChix,
                                          double pChiy, double mMother,
                                          double mInvis) {
    const double lambda =
        0.5 * (mMother * mMother - vis.m2() - mInvis * mInvis) +
        vis.px() * pChix + vis.py() * pChiy;
    const double etVis = vis.transverseEnergy();
    const double etChi =
        std::sqrt(pChix * pChix + pChiy * pChiy + mInvis * mInvis);
...
std::vector<std::pair<FourMomentum, FourMomentum>> mat::maos(
    ...
    auto mt2sol = ben_findsols(mt2, p1.px(), p1.py(), p1.m(), mInvis, p2.px(),
                               p2.py(), metx, mety, p2.m(), mInvis);
